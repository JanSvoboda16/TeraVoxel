...
{
public:
	__host__ __device__ static bool ComputeRayIntersection(const Vector3f& rayDireciton, const Vector3f& cameraPosition, const Vector3f& volumeDimensions, Vector3f& start, Vector3f& stop)
	{
		...
		Vector3f maxIndexes(volumeDimensions[0] - 1, volumeDimensions[1] - 1, volumeDimensions[2] - 1);
			...
			position[0] <= maxIndexes[0] && position[1] <= maxIndexes[1] && position[2] <= maxIndexes[2])
		{
			...
			else
			{
				...
				uint8_t farestIndex = 0;

				for (uint8_t i = 1; i < intersectionsCount; i++)
				{
