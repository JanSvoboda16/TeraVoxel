...
public:
	...
	__host__  Camera(const Vector3f& observerCenter, int observerDistance, const Vector3f& voxelDimensions, int width, int height, float viewAngle, float nearPlaneDistance = 20, float farPlaneDiscance = 10000, std::shared_ptr<MeshNode> orbiterMeshNode = nullptr);
	...
	__host__ void ChangePosition(const Vector3f& position);
	...
	__host__ void ChangeObserverAxis(char axis, bool rotate);
	...
	__host__ void Observe(float deltaXAngle, float deltaYAngle, float deltaDistance, float deltaXCenter, float deltaYCenter, float deltaZCenter);
	...
	__host__ __device__ Vector3f GetShrankRayDirection(int xPixel, int yPixel);
	...
	__host__ __device__ Vector3f ShrinkVector(Vector3f vector) { return vector.array() * _correction.array(); }
	...
	__host__ __device__ Vector3f DeshrinkVector(Vector3f vector) { return vector.array() * _voxelDimensions.array(); }
	...
	__host__ __device__ void ChangeScreenSize(int width, int height);

	/// <summary>
	/// Gets size of the screen in pixels
	/// </summary>
	/// <returns>size of the screen</returns>
	__host__ __device__ Vector2i GetScreenSize() { return Vector2i(_screenWidth, _screenHeight); }

	__host__ __device__ int32_t GetScreenWidth() { return _screenWidth; }

	__host__ __device__ int32_t GetScreenHieght() { return _screenHeight; }

	/// <summary>
	/// Gets the camera view angle
	/// </summary>
	/// <returns></returns>
	__host__ __device__ float GetViewAngle() { return _viewAngle; }

	/// <summary>
	/// Sets the camera view angle
	/// </summary>
	/// <param name="viewAngle"></param>
	__host__ void SetViewAngle(float viewAngle) { _viewAngle = viewAngle; RecomputeParams(); }
	...
	/// <returns></returns>
	__host__ __device__ Vector3f GedDistanceFromProjected(float zValue, int xPixel, int yPixel);

	/// <summary>
	/// Gets the camera position
	/// </summary>
	/// <returns>position</returns>
	__host__ __device__ Vector3f GetPosition() { return _position; }

	/// <summary>
	/// Sets the farplane distance
	/// </summary>
	/// <param name="farPlaneDistance"></param>
	__host__ void SetFarPlaneDistance(float farPlaneDistance) { _farPlaneDistance = farPlaneDistance; RecomputeParams(); }

	/// <summary>
	/// Sets the nearplane distance
	/// </summary>
	/// <param name="nearPlaneDistance"></param>
	__host__ void SetNearPlaneDistance(float nearPlaneDistance) { _nearPlaneDistance = nearPlaneDistance; RecomputeParams(); }

	/// <summary>
	/// Gets the farplane distance
	/// </summary>
	/// <returns>distance</returns>
	__host__ __device__ float GetFarPlaneDistance() { return _farPlaneDistance; }

	/// <summary>
	/// Gets the nearplane distance
	/// </summary>
	/// <returns>distance</returns>
	__host__ __device__ float GetNearPlaneDistance() { return _nearPlaneDistance; }
