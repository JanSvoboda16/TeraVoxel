...
public:
	...
	static void Set(std::shared_ptr<IVolumeScene> scene, std::shared_ptr<VolumeVisualizerSettingsBase> settings) {
		...
		std::shared_ptr<IVolumeVisualizerFactory<T>> visfac = std::shared_ptr<IVolumeVisualizerFactory<T>>((IVolumeVisualizerFactory<T>*) new EmptyVolumeVisualizerFactory<T>(std::static_pointer_cast<EmptyVolumeVisualizerSettings>(settings)));
